...
from core.api_protection import APICircuitBreaker, CircuitBreakerConfig, CircuitOpenError
...
class TestCircuitBreakerRetryIntegration:
    ...
    def retry_manager(self):
        ...
        return RetryManager({"max_retries": 2, "backoff_base": 0.01})  # Fast retries for testing
    ...
    def test_retry_aborts_when_circuit_open_before_first_attempt(
        self, retry_manager, circuit_breaker, mock_execution_func, mock_signal, mock_context
    ):
        ...
        with pytest.raises(CircuitOpenError, match="Circuit is open.*aborting retry"):
            asyncio.run(retry_manager.execute_with_retry(
                mock_execution_func,
                mock_signal,
    ...
    def test_retry_continues_normally_when_circuit_closed(
        ...
        result = asyncio.run(retry_manager.execute_with_retry(
