...
class TestPriceGapDetector:
    ...
    def test_detect_normal_price_change(self):
        ...
        data = pd.DataFrame({"close": [100.0, 101.0, 102.0]})  # Normal 1-2% changes
    ...
    def test_detect_price_gap(self):
        ...
        data = pd.DataFrame({"close": [100.0, 115.0]})  # 15% gap (above 5% threshold)
    ...
    def test_detect_small_gap(self):
        ...
        data = pd.DataFrame({"close": [100.0, 102.0]})  # 2% gap (below threshold)
...
class TestAnomalyDetector:
    ...
    def test_check_signal_anomaly_skip_trade(self):
        ...
        data = pd.DataFrame(
            {"close": [100.0, 120.0], "volume": [1000, 1000]}  # 20% gap
        )
    ...
    def test_check_signal_anomaly_scale_down(self):
        ...
        data = pd.DataFrame(
            {"close": [100 + i * 0.1 for i in range(30)], "volume": volumes},
            index=dates,
        )
...
class TestAnomalyLogging:
    ...
    def test_log_to_file(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            detector = AnomalyDetector({"logging": {"file": log_file}})
            ...
            log_entry = AnomalyLog(
                symbol="TEST", anomaly_result=anomaly_result, action_taken="skip_trade"
            )
    ...
    def test_log_to_json(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            detector = AnomalyDetector({"logging": {"json_file": json_file}})
    ...
    def test_trade_logger_integration(self, mock_trade_logger):
        ...
        detector._log_anomaly(
            "TEST", anomaly_result, data, None, AnomalyResponse.LOG_ONLY
        )
...
class TestEdgeCases:
    ...
    def test_detector_with_nan_values(self):
        ...
        data = pd.DataFrame(
            {"close": [100, float("nan"), 102], "volume": [1000, float("nan"), 1002]}
        )
    ...
    def test_zero_std_deviation(self):
        ...
        data = pd.DataFrame({"close": [100] * 60})  # Same price for all periods
    ...
    def test_extreme_values(self):
        ...
        data = pd.DataFrame({"close": [100, 1000000]})  # Extreme gap
...
class TestConfiguration:
    ...
    def test_custom_configuration(self):
        ...
        config = {
            "enabled": False,
            "price_zscore": {"enabled": False, "z_threshold": 2.0},
            "response": {"scale_down_factor": 0.3},
