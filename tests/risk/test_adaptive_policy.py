...
class TestMarketConditionMonitor:
    ...
    def test_assess_market_conditions_high_volatility(self):
        ...
        config = {'volatility_threshold': 0.5}  # Very high threshold to ensure detection
        monitor = MarketConditionMonitor(config)

        # Create high volatility data with extreme swings
        data = pd.DataFrame({
            'high': [100, 200, 50, 300, 25, 400, 10, 500, 5, 600,
                    2, 700, 1, 800, 0.5, 900, 0.1, 1000, 0.05, 1100],
            'low': [99, 1, 150, 1, 200, 1, 250, 1, 300, 1,
                   350, 1, 400, 1, 450, 1, 500, 1, 550, 1],
            'close': [99.5, 150, 100, 200, 150, 300, 200, 400, 250, 500,
                     300, 600, 350, 700, 400, 800, 450, 900, 500, 1000],
            'volume': [1000] * 20
        })
        ...
        assert conditions['volatility_level'] in ['low', 'moderate', 'high', 'very_high', 'unknown']
        assert conditions['risk_level'] in [level.value for level in RiskLevel]
...
class TestPerformanceMonitor:
    ...
    def test_consecutive_losses_tracking(self):
        ...
        # Add winning trade
        monitor.update_performance({'pnl': 100.0, 'timestamp': datetime.now().isoformat()})
        assert monitor.consecutive_losses == 0

        # Add losing trade
        monitor.update_performance({'pnl': -50.0, 'timestamp': datetime.now().isoformat()})
        assert monitor.consecutive_losses == 1

        # Add another losing trade
        monitor.update_performance({'pnl': -75.0, 'timestamp': datetime.now().isoformat()})
        assert monitor.consecutive_losses == 2

        # Add winning trade (reset counter)
        monitor.update_performance({'pnl': 200.0, 'timestamp': datetime.now().isoformat()})
        assert monitor.consecutive_losses == 0
...
class TestAdaptiveRiskPolicy:
    ...
    def test_get_risk_multiplier_high_volatility(self):
        ...
        data = pd.DataFrame({
            'high': [100, 150, 80, 160, 70, 170, 60, 180, 50, 190,
                    40, 200, 30, 210, 20, 220, 10, 230, 5, 240],
            'low': [90, 50, 120, 40, 130, 30, 140, 20, 150, 10,
                   160, 5, 170, 2, 180, 1, 190, 0.5, 200, 0.1],
            'close': [95, 120, 100, 140, 90, 150, 80, 160, 70, 170,
                     60, 180, 50, 190, 40, 200, 30, 210, 20, 220],
            'volume': [1000] * 20
        })
        ...
        has_risk_indicator = (
            'high' in reasoning_lower or
            'risk' in reasoning_lower or
            'volatility' in reasoning_lower or
            'market' in reasoning_lower
        )
        assert has_risk_indicator, f"Reasoning should indicate risk assessment: {reasoning}"
    ...
    def test_get_risk_multiplier_poor_performance(self):
        ...
        assert multiplier < 1.0
        assert 'consecutive losses' in reasoning.lower() or 'defensive mode' in reasoning.lower()
    ...
    def test_kill_switch_activation(self):
        ...
        for i in range(4):
            policy.defensive_mode_history.append({
                'mode': DefensiveMode.DEFENSIVE.value,
...
class TestIntegrationWithPositionSizing:
    ...
    def test_get_adaptive_risk_policy_function(self, mock_policy_class, mock_get_config):
