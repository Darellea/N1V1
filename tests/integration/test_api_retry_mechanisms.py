...
class TestAPIRetryMechanisms:
    ...
    def data_fetcher_config(self):
        ...
        return {
            ...
            "endpoint_retry_configs": {
                "fetch_ohlcv": {"max_attempts": 5, "base_delay": 0.2}
            },
        }
        ...
        with pytest.raises((asyncio.TimeoutError, Exception)):
            ...
            async def fetch_with_timeout():
                return await asyncio.wait_for(
                    data_fetcher.get_historical_data("BTC/USDT"), timeout=2.0
                )
    ...
    async def test_endpoint_specific_retry_config(self, data_fetcher, mock_exchange):
        ...
        result = await data_fetcher.get_realtime_data(
    ...
    @pytest.mark.asyncio
    async def test_graceful_degradation_on_api_unavailable(
        self, data_fetcher, mock_exchange
    ):
    ...
    @pytest.mark.asyncio
    async def test_request_deduplication_for_idempotent_operations(
        self, data_fetcher, mock_exchange
    ):
    ...
    @pytest.mark.asyncio
    async def test_permanent_vs_temporary_failure_handling(
        self, data_fetcher, mock_exchange
    ):
    ...
    async def test_circuit_breaker_recovery(self, data_fetcher, mock_exchange):
        ...
        data_fetcher.circuit_breaker.config.success_threshold = (
            1  # Close after 1 success
        )
        ...
        data_fetcher.circuit_breaker.last_failure_time = (
            time.time() - 1.0
        )  # 1 second ago
