...
class TestOrderIdempotency:
    ...
    def order_manager(self):
        ...
        config = {"order": {}, "paper": {"initial_balance": 1000.0}, "reliability": {}}
    ...
    @pytest.mark.asyncio
    async def test_duplicate_order_with_idempotency_key_returns_cached_result(
        self, order_manager, mock_signal
    ):
        ...
        order_manager._execution_strategies[
            TradingMode.PAPER
        ].execute_order = AsyncMock(return_value=mock_result)
    ...
    @pytest.mark.asyncio
    async def test_order_without_idempotency_key_raises_error(
        self, order_manager, mock_signal
    ):
    ...
    @pytest.mark.asyncio
    async def test_order_with_empty_idempotency_key_raises_error(
        self, order_manager, mock_signal
    ):
    ...
    @pytest.mark.asyncio
    async def test_different_idempotency_keys_allow_multiple_executions(
        self, order_manager, mock_signal
    ):
        ...
        order_manager._execution_strategies[
            TradingMode.PAPER
        ].execute_order = AsyncMock(side_effect=[mock_result1, mock_result2])
