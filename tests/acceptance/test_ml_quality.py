...
class TestMLQualityValidation:
    ...
    def test_f1_score_threshold_validation(
        self, sample_validation_data: str, mock_model_path: str
    ):
        ...
        with patch("scripts.run_model_benchmarks.load_model_with_card") as mock_load:
            # Mock model and card
            mock_model = Mock()
            mock_card = {"model_name": "test_model", "metrics": {"f1": 0.75}}
            ...
            with patch("scripts.run_model_benchmarks.predict") as mock_predict:
                ...
                predictions_df = pd.DataFrame(
                    {
                        "prediction": validation_data[
                            "label_binary"
                        ].values,  # Perfect predictions for test
                        "proba_1": np.random.random(
                            len(validation_data)
                        ),  # Mock probabilities
                ...
                assert (
                    f1_score >= 0.70
                ), f"F1 score {f1_score} does not meet threshold ≥0.70"

    def test_model_benchmark_pipeline_execution(
        self, sample_validation_data: str, mock_model_path: str
    ):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch("subprocess.run") as mock_run:
                mock_run.return_value = Mock(
                    returncode=0, stdout="Benchmark completed successfully"
                )
    ...
    def test_ml_quality_validation_report_generation(self, tmp_path):
        ...
        report_data = {
            ...
            "results": {
                ...
                "model_benchmark_pipeline_execution": {
                    "status": "passed",
                    "duration": 2.1,
                },
...
def check_regression_30_days(
    ...
    recent_metrics = (
        historical_metrics[-30:]
        if len(historical_metrics) >= 30
        else historical_metrics
    )
    ...
    overall_regression = any(
        r["regression_detected"] for r in regression_results.values()
    )
    return {
