...
class TestMainFunction:
    ...
    def test_main_success(
        ...
        # Mock configuration
        mock_config = {
            "predictive_models": {"enabled": True, "models": ["price_predictor"]}
        }
    ...
    def test_main_predictive_models_disabled(self, mock_parse_args, mock_load_config):
        ...
        mock_config = {"predictive_models": {"enabled": False}}
        mock_load_config.return_value = mock_config
    ...
    @patch("ml.train.load_historical_data")
    @patch("argparse.ArgumentParser.parse_args")
    def test_main_training_failure(
        self, mock_parse_args, mock_load_data, mock_load_config
    ):
...
class TestConfusionMatrixGeneration:
    ...
    def test_confusion_matrix_with_perfect_predictions(self, tmp_path):
        ...
        assert (
            confusion_matrix_path.exists()
        ), "Perfect confusion matrix file should be created"
...
class TestReproducibility:
    ...
    def test_set_deterministic_seeds_basic(
        self, mock_pandas_random, mock_numpy_seed, mock_random_seed
    ):
    ...
    def test_set_deterministic_seeds_libs_unavailable(self, mock_logger):
        ...
        with patch.dict(
            "sys.modules", {"sklearn": None, "torch": None, "tensorflow": None}
        ):
...
class TestModelLoaderReproducibility:
    ...
    def test_load_model_from_registry_success(
        self, mock_start_run, mock_set_experiment, mock_client_class, mock_load_model
    ):
        ...
        def mock_exists(path):
            return path in [
                ...
                "models/test_model.pkl_card.json",
            ]
            ...
            with patch("builtins.open", mock_open(read_data='{"test": "data"}')):
                with patch("os.remove"):  # Mock file cleanup
                    with patch("ml.model_loader.load_model") as mock_load_model_func:
                        ...
                        model, card = load_model_from_registry(
                            "test_model", version="1"
                        )
    ...
    @patch("mlflow.set_experiment")
    @patch("mlflow.start_run")
    def test_load_model_from_registry_fallback(
        self,
        mock_start_run,
        mock_set_experiment,
        mock_load_with_card,
