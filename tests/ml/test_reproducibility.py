...
class TestEnvironmentSnapshot:
    ...
    def test_capture_environment_snapshot_basic_info(self, mock_platform, mock_sys):
        ...
        mock_platform.version.return_value = "#83-Ubuntu SMP Sat May 8 02:35:39 UTC 2021"
        mock_platform.machine.return_value = "x86_64"
        mock_platform.node.return_value = "test-hostname"

        env_snapshot = capture_environment_snapshot()

        assert env_snapshot['python_version'] == "3.9.0 (default, Oct  1 2021, 00:00:00)"
        assert env_snapshot['platform'] == "Linux-5.4.0-74-generic-x86_64-with-glibc2.29"
    ...
    def test_capture_environment_snapshot_env_vars(self, mock_os):
        ...
        assert env_snapshot['environment_variables']['PYTHONPATH'] == '/usr/local/lib/python3.9'
...
class TestExperimentTrackerReproducibility:
    ...
    def test_log_parameters_with_reproducibility(self):
        ...
        assert 'python_random' in tracker.metadata['reproducibility']['libraries_with_seeds']
        assert tracker.metadata['reproducibility']['environment_snapshot']['python_version'] == '3.9.0'
...
class TestReproducibilityValidation:
    ...
    def test_reproducibility_metadata_structure(self):
        ...
        required_fields = [
            'random_seed', 'deterministic_mode', 'libraries_with_seeds',
            'environment_snapshot', 'platform_info'
        ]
