...
class TestXGBoostModel:
    ...
    @pytest.mark.skipif(
        "not hasattr(__import__('ml.ml_filter'), 'XGBOOST_AVAILABLE') or not __import__('ml.ml_filter').XGBOOST_AVAILABLE"
    )
    def test_init(self):
        """Test initialization."""
        model = XGBoostModel()
...
class TestLightGBMModel:
    ...
    @pytest.mark.skipif(
        "not hasattr(__import__('ml.ml_filter'), 'LIGHTGBM_AVAILABLE') or not __import__('ml.ml_filter').LIGHTGBM_AVAILABLE"
    )
    def test_init(self):
        """Test initialization."""
        model = LightGBMModel()
...
class TestMLFilter:
    ...
    def test_fit(self):
        ...
        X = pd.DataFrame(
            {"feature1": list(range(1, 21)), "feature2": [i * 2 for i in range(1, 21)]}
        )
    ...
    def test_filter_signal_buy_approved(self):
        ...
        # Filter buy signal
        result = ml_filter.filter_signal(
            X.iloc[[-1]], "buy"
        )  # Last sample is class 1 (buy)
...
class TestFactoryFunctions:
    ...
    def test_create_ml_filter_with_config(self):
        ...
        config = {"model_config": {"C": 0.5}, "confidence_threshold": 0.7}
