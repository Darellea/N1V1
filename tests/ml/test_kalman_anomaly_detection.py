...
class TestKalmanAnomalyDetector:
    ...
    def test_detect_normal_market_conditions(self):
        ...
        assert (
            result.anomaly_type == AnomalyType.KALMAN_FILTER
        )  # Kalman detector always returns KALMAN_FILTER type
    ...
    def test_adaptive_threshold_volatility_regime(self):
        ...
        assert (
            result_high_vol.confidence_score < 0.8
        )  # Should be less confident due to adaptation
...
class TestKalmanAnomalyDetectorAccuracy:
    ...
    def test_false_positive_reduction(self):
        ...
        assert (
            anomaly_count < 10
        )  # Should not detect too many anomalies in normal volatility
...
class TestKalmanIntegration:
    ...
    def test_kalman_detection_workflow(self):
        ...
        kalman_anomalies = [
            a for a in anomalies if a.anomaly_type == AnomalyType.KALMAN_FILTER
        ]
        assert (
            len(kalman_anomalies) >= 0
