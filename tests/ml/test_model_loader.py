...
from ml.model_loader import _align_features, load_model, load_model_with_card, predict
...
class TestModelLoader:
    ...
    def test_predict_with_proba(self, mock_model, sample_features):
        ...
        pd.testing.assert_series_equal(
            result["proba_0"], pd.Series(expected_proba_0, name="proba_0")
        )
        pd.testing.assert_series_equal(
            result["proba_1"], pd.Series(expected_proba_1, name="proba_1")
        )
    ...
    def test_predict_without_proba(self, sample_features):
        ...
        assert "prediction" in result.columns
        assert "confidence" in result.columns
        assert result["confidence"].equals(
            pd.Series([1.0, 1.0, 1.0], name="confidence")
        )
    ...
    def test_align_features_missing_features(self, sample_features):
        ...
        with patch("ml.model_loader.logger") as mock_logger:
            ...
            assert result["missing_feature"].equals(
                pd.Series([0.0, 0.0, 0.0], name="missing_feature")
            )
    ...
    def test_predict_with_classes_inference(self, sample_features):
        ...
        class MockModel:
            def __init__(self):
                ...
                self._predict_proba_result = np.array(
                    [[0.2, 0.8], [0.7, 0.3], [0.1, 0.9]]
                )
                ...
                with patch("os.path.exists") as mock_exists:
                    with patch(
                        ...
                        mock_exists.side_effect = (
                            lambda path: path == expected_card_path
                        )
