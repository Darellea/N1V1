...
class TestAllocationEngine:
    ...
    def test_initialization(self, allocation_engine: AllocationEngine, allocation_config: Dict[str, Any]):
    ...
    def test_calculate_equal_weights(self, allocation_engine: AllocationEngine):
        ...
        for strategy_id, allocation in allocations.items():
            ...
            assert allocation['capital_allocated'] == Decimal('10000') * Decimal(str(allocation['weight']))
    ...
    def test_calculate_sharpe_weights(self, allocation_engine: AllocationEngine):
        ...
        strategy_performance = {
            'high_sharpe': [
                {'daily_return': 0.05, 'pnl': 500.0, 'trades': 10, 'timestamp': base_time},
                {'daily_return': 0.04, 'pnl': 400.0, 'trades': 10, 'timestamp': base_time + timedelta(days=1)},
                {'daily_return': 0.06, 'pnl': 600.0, 'trades': 10, 'timestamp': base_time + timedelta(days=2)},
                {'daily_return': 0.03, 'pnl': 300.0, 'trades': 10, 'timestamp': base_time + timedelta(days=3)},
                {'daily_return': 0.05, 'pnl': 500.0, 'trades': 10, 'timestamp': base_time + timedelta(days=4)}
            ],
            'low_sharpe': [
                {'daily_return': 0.01, 'pnl': 100.0, 'trades': 10, 'timestamp': base_time},
                {'daily_return': 0.015, 'pnl': 150.0, 'trades': 10, 'timestamp': base_time + timedelta(days=1)},
                {'daily_return': 0.008, 'pnl': 80.0, 'trades': 10, 'timestamp': base_time + timedelta(days=2)},
                {'daily_return': 0.012, 'pnl': 120.0, 'trades': 10, 'timestamp': base_time + timedelta(days=3)},
                {'daily_return': 0.009, 'pnl': 90.0, 'trades': 10, 'timestamp': base_time + timedelta(days=4)}
            ]
        }
        ...
        assert allocations['high_sharpe']['weight'] > allocations['low_sharpe']['weight']
        assert abs(allocations['high_sharpe']['weight'] + allocations['low_sharpe']['weight'] - 1.0) < 0.01
    ...
    def test_weight_constraints(self, allocation_engine: AllocationEngine):
        ...
        strategy_performance = {
            'excellent': [{'daily_return': 0.10, 'pnl': 1000.0, 'trades': 10}],  # Very high return
            'poor': [{'daily_return': -0.05, 'pnl': -500.0, 'trades': 10}],     # Very low return
            'mediocre': [{'daily_return': 0.01, 'pnl': 100.0, 'trades': 10}]   # Normal return
        }
        ...
        for strategy_id, allocation in allocations.items():
            assert allocation_engine.min_weight <= allocation['weight'] <= allocation_engine.max_weight
    ...
    def test_insufficient_performance_data(self, allocation_engine: AllocationEngine):
        ...
        strategy_performance = {
            'new_strategy': [{'daily_return': 0.01, 'pnl': 10.0, 'trades': 1}]  # Only 1 data point
        }
    ...
    def test_should_rebalance_positive(self, allocation_engine: AllocationEngine):
        ...
        should_rebalance = allocation_engine.should_rebalance(current_allocations, new_allocations)
    ...
    def test_should_rebalance_missing_strategy(self, allocation_engine: AllocationEngine):
    ...
    def test_sortino_weighted_allocation(self, allocation_engine: AllocationEngine):
        ...
        strategy_performance = {
            'low_downside': [
                {'daily_return': 0.03, 'pnl': 300.0, 'trades': 10, 'timestamp': base_time},
                {'daily_return': 0.04, 'pnl': 400.0, 'trades': 10, 'timestamp': base_time + timedelta(days=1)},
                {'daily_return': 0.02, 'pnl': 200.0, 'trades': 10, 'timestamp': base_time + timedelta(days=2)},
                {'daily_return': 0.035, 'pnl': 350.0, 'trades': 10, 'timestamp': base_time + timedelta(days=3)},
                {'daily_return': 0.025, 'pnl': 250.0, 'trades': 10, 'timestamp': base_time + timedelta(days=4)}
            ],
            'high_downside': [
                {'daily_return': 0.02, 'pnl': 200.0, 'trades': 10, 'timestamp': base_time},
                {'daily_return': -0.03, 'pnl': -300.0, 'trades': 10, 'timestamp': base_time + timedelta(days=1)},  # Loss
                {'daily_return': 0.04, 'pnl': 400.0, 'trades': 10, 'timestamp': base_time + timedelta(days=2)},
                {'daily_return': -0.02, 'pnl': -200.0, 'trades': 10, 'timestamp': base_time + timedelta(days=3)},  # Loss
                {'daily_return': 0.03, 'pnl': 300.0, 'trades': 10, 'timestamp': base_time + timedelta(days=4)}
            ]
        }
        ...
        assert allocations['low_downside']['weight'] > allocations['high_downside']['weight']
