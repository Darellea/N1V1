...
def mock_ohlcv_data():
    ...
    return [
        [1609459200000, 100.0, 102.0, 98.0, 101.0, 1000.0],  # [timestamp, o, h, l, c, v]
...
async def data_fetcher(mock_exchange_config):
    ...
    fetcher.session = aiohttp.ClientSession()  # Real session but won't make actual calls
    yield fetcher
...
async def test_fetch_ohlcv_data(data_fetcher, mock_ohlcv_data):
    ...
    data_fetcher.exchange.fetch_ohlcv.return_value = mock_ohlcv_data
    ...
    data_fetcher.exchange.fetch_ohlcv.side_effect = Exception("API Error")
...
async def test_rate_limiting(data_fetcher):
    ...
    data_fetcher.exchange.fetch_ohlcv.return_value = []
...
async def test_data_validation(data_fetcher, mock_ohlcv_data):
    ...
    data_fetcher.exchange.fetch_ohlcv.return_value = bad_data
    df = await data_fetcher.get_historical_data('BTC/USDT', '1h')
    assert df.empty
...
async def test_historical_data_validation(historical_loader):
    ...
    invalid_timeframe_data.index = [datetime(2023,1,1), datetime(2023,1,3)]  # 2-day gap for 1d timeframe
...
def test_cache_operations(data_fetcher, mock_ohlcv_data):
    ...
    df = pd.DataFrame(mock_ohlcv_data, columns=['timestamp', 'open', 'high', 'low', 'close', 'volume'])
