...
class TestValidationAsset:
    ...
    def test_str_representation(self):
        ...
        asset = ValidationAsset(symbol="LINK/USDT", name="Chainlink", weight=0.9)
...
class TestAssetSelector:
    ...
    def test_initialization_default_config(self):
        ...
        with patch(
            "optimization.asset_selector.get_cross_asset_validation_config"
        ) as mock_config:
    ...
    def test_select_validation_assets_no_candidates(self):
        ...
        config = self.config.copy()
        config["validation_assets"] = [{"symbol": "BTC/USDT", "name": "Bitcoin"}]
    ...
    async def test_select_validation_assets_with_correlation_filter(self):
        ...
        mock_data.__getitem__.return_value.pct_change.return_value.dropna.return_value = (
            MagicMock()
        )
        ...
        with patch.object(selector, "_filter_correlated_assets") as mock_filter:
            mock_filter.return_value = selector.available_assets[
                :2
            ]  # Return first 2 assets
    ...
    def test_get_market_cap_weights_from_config(self):
        ...
        config = self.config.copy()
        config["market_cap_weights"] = {"ETH/USDT": 0.6, "ADA/USDT": 0.4}
        ...
        with patch.object(
            selector, "_fetch_market_caps_dynamically", return_value=None
        ):
    ...
    def test_fetch_market_caps_dynamically_coinmarketcap_fallback(self):
        ...
        with patch.object(
            selector, "_fetch_from_coingecko", return_value=None
    ...
    def test_fetch_market_caps_dynamically_local_cache_fallback(self):
        ...
        with patch.object(
            ...
            selector, "_fetch_from_coinmarketcap", return_value=None
        ), patch.object(
            selector, "_fetch_from_local_cache"
        ) as mock_cache:
    ...
    async def test_fetch_from_coingecko_async_success(self):
        ...
        mock_response_data = [{"id": "ethereum", "market_cap": 50000000000}]
    ...
    def test_fetch_from_coinmarketcap_success(self):
        ...
        mock_response_data = {
            "data": {"1027": {"quote": {"USD": {"market_cap": 30000000000}}}}
        }
    ...
    def test_fetch_from_local_cache_success(self):
        ...
        cache_data = {
            "timestamp": time.time(),
            "market_caps": {"ETH/USDT": 40000000000, "ADA/USDT": 20000000000},
        }
    ...
    def test_update_asset_weights(self):
        ...
        new_weights = {"ETH/USDT": 0.8, "ADA/USDT": 0.2}

        selector.update_asset_weights(new_weights)

        eth_asset = next(
            asset for asset in selector.available_assets if asset.symbol == "ETH/USDT"
        )
        ada_asset = next(
            asset for asset in selector.available_assets if asset.symbol == "ADA/USDT"
        )
    ...
    def test_remove_validation_asset(self):
        ...
        assert not any(
            asset.symbol == "ADA/USDT" for asset in selector.available_assets
        )
    ...
    async def test_filter_correlated_assets_async(self):
        ...
        primary_data.__getitem__.return_value.pct_change.return_value.dropna.return_value = (
            primary_returns
        )
        ...
        asset_data.__getitem__.return_value.pct_change.return_value.dropna.return_value = (
            asset_returns
        )
        ...
        filtered = await selector._filter_correlated_assets_async(
            candidates, "BTC/USDT", mock_fetcher
        )
