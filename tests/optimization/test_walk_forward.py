...
def optimizer_with_bounds(basic_config):
    ...
    optimizer.add_parameter_bounds(ParameterBounds(
        name="fast_period",
        min_value=5,
        max_value=20,
        param_type="int"
...
class TestWalkForwardDataSplitter:
    ...
    def test_split_data_no_overlap(self, sample_data):
        ...
        for i in range(1, len(windows)):
            ...
            assert time_diff >= -3600, f"Windows {i-1} and {i} overlap significantly"  # Allow 1 hour tolerance
...
class TestWalkForwardOptimizer:
    ...
    def test_optimize_with_windows(self, basic_config, sample_data):
        ...
        with patch.object(optimizer, '_evaluate_strategy') as mock_eval, \
             patch.object(optimizer, '_create_optimizer') as mock_create:

            # Mock optimizer instance
            mock_optimizer_instance = Mock()
            mock_optimizer_instance.optimize.return_value = {'fast_period': 10, 'slow_period': 25}
    ...
    def test_calculate_aggregate_metrics(self, basic_config):
        ...
        for i in range(3):
            window = WalkForwardWindow(
                ...
                test_metrics={'sharpe_ratio': 1.2 + i * 0.3, 'total_return': 0.1 + i * 0.05},
    ...
    def test_save_results(self, basic_config, tmp_path):
        ...
        # Create mock windows
        optimizer.windows = [WalkForwardWindow(
            window_index=0,
            train_start=datetime.now(),
            train_end=datetime.now(),
            test_start=datetime.now(),
...
class TestConvenienceFunctions:
    ...
    def test_run_walk_forward_analysis(self, sample_data):
        ...
        with patch('optimization.walk_forward.create_walk_forward_optimizer') as mock_create:
            # Mock the optimizer
            mock_optimizer = Mock()
...
class TestIntegration:
    ...
    def test_full_walk_forward_workflow(self, sample_data, tmp_path):
        ...
        with patch.object(optimizer, '_create_optimizer') as mock_create, \
             patch.object(optimizer, '_evaluate_strategy') as mock_eval:
            ...
            assert isinstance(best_params, dict)
            assert len(optimizer.windows) > 0
            assert optimizer.aggregate_metrics['total_windows'] == len(optimizer.windows)
