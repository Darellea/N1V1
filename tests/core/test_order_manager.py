...
class TestOrderManager:
    ...
    def mock_executors(self):
        ...
        with patch("core.order_manager.LiveOrderExecutor") as mock_live, patch(
            ...
            mock_backtest_instance.execute_backtest_order = AsyncMock(
                return_value={"id": "backtest_order", "status": "filled"}
            )
    ...
    @pytest.mark.asyncio
    async def test_execute_order_paper_mode(
    ...
    @pytest.mark.asyncio
    async def test_execute_order_backtest_mode(
        self, config, mock_executors, mock_managers
    ):
        ...
        assert result is not None
        mock_executors["backtest"].execute_backtest_order.assert_called_once_with(
            signal
        )

    @pytest.mark.asyncio
    async def test_execute_order_safe_mode_active(
        self, config, mock_executors, mock_managers
    ):
    ...
    @pytest.mark.asyncio
    async def test_execute_order_live_mode_with_retry(
        self, config, mock_executors, mock_managers
    ):
        ...
        mock_executors["live"].execute_live_order = AsyncMock(
            return_value={"id": "live_order", "status": "filled"}
        )
    ...
    async def test_cancel_order_live_mode(self, config, mock_executors, mock_managers):
        ...
        )  # Add fetch_ticker for balance calculation
    ...
    @pytest.mark.asyncio
    async def test_cancel_order_non_live_mode(
        self, config, mock_executors, mock_managers
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_balance_backtest_mode(
        self, config, mock_executors, mock_managers
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_equity_with_portfolio_mode(
        self, config, mock_executors, mock_managers
    ):
    ...
    async def test_initialize_portfolio(self, config, mock_executors, mock_managers):
        ...
        mock_executors["paper"].set_portfolio_mode.assert_called_once_with(
            True, pairs, allocation
        )
        mock_managers["portfolio"].initialize_portfolio.assert_called_once_with(
            pairs, True, allocation
        )
    ...
    @pytest.mark.asyncio
    async def test_execute_order_safe_mode_trigger_counter(
        self, config, mock_executors, mock_managers
    ):
    ...
    @pytest.mark.asyncio
    async def test_execute_order_unknown_mode(
        self, config, mock_executors, mock_managers
    ):
    ...
    @pytest.mark.asyncio
    async def test_cancel_order_with_exception(
        self, config, mock_executors, mock_managers
    ):
    ...
    @pytest.mark.asyncio
    async def test_cancel_all_orders_with_exception(
        self, config, mock_executors, mock_managers
    ):
        ...
        mock_managers["processor"].open_orders = {
            "order1": MagicMock(),
            "order2": MagicMock(),
        }
    ...
    async def test_rate_limit(self, config, mock_executors, mock_managers):
        ...
        assert elapsed2 >= 0.08, f"Second rate limit call too fast: {elapsed2:.4f}s"

    @pytest.mark.asyncio
    async def test_rate_limit_deterministic_simulation(
        self, config, mock_executors, mock_managers
    ):
        ...
        with patch("asyncio.sleep", side_effect=instant_sleep):
            ...
            assert (
                elapsed < 0.01
            ), f"First call should be fast with mocked sleep: {elapsed}"
    ...
    @pytest.mark.asyncio
    async def test_get_cached_ticker_cache_hit(
        self, config, mock_executors, mock_managers
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_cached_ticker_live_mode(
        self, config, mock_executors, mock_managers
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_cached_ticker_non_live_mode(
        self, config, mock_executors, mock_managers
    ):
        ...
        with pytest.raises(
            ValueError, match="Ticker fetching only supported in LIVE mode"
        ):
            await om._get_cached_ticker("BTC/USDT")

    @pytest.mark.asyncio
    async def test_get_balance_live_mode_with_exception(
        self, config, mock_executors, mock_managers
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_balance_backtest_portfolio_mode(
        self, config, mock_executors, mock_managers
    ):
        """Test get_balance in backtest mode with portfolio."""
        om = OrderManager(config, TradingMode.BACKTEST)
        om.portfolio_mode = True
        mock_managers["portfolio"].paper_balances = {
            "USDT": Decimal("5000"),
            "BTC": Decimal("0.1"),
        }
    ...
    @pytest.mark.asyncio
    async def test_get_equity_live_mode_with_positions(
        self, config, mock_executors, mock_managers
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_equity_with_invalid_position_data(
        self, config, mock_executors, mock_managers
    ):
    ...
    @pytest.mark.asyncio
    async def test_initialize_portfolio_invalid_inputs(
        self, config, mock_executors, mock_managers
    ):
        ...
        with pytest.raises(ValueError, match="exceeds 100%"):
            await om.initialize_portfolio(
                ["BTC/USDT", "ETH/USDT"], True, {"BTC/USDT": 0.7, "ETH/USDT": 0.4}
            )

    @pytest.mark.asyncio
    async def test_initialize_portfolio_equal_allocation(
        self, config, mock_executors, mock_managers
    ):
    ...
    def test_validate_order_payload_invalid_schema(
        self, config, mock_executors, mock_managers
    ):
    ...
    def test_validate_order_payload_invalid_symbol_format(
        self, config, mock_executors, mock_managers
    ):
    ...
    def test_validate_order_payload_business_rules_negative_amount(
        self, config, mock_executors, mock_managers
    ):
    ...
    def test_validate_order_payload_business_rules_stop_without_loss(
        self, config, mock_executors, mock_managers
    ):
    ...
    def test_validate_order_payload_business_rules_invalid_signal_order_combo(
        self, config, mock_executors, mock_managers
    ):
        ...
        with pytest.raises(
            ValueError, match="Entry signals should use MARKET or LIMIT"
        ):
            om._validate_order_payload(invalid_signal)

    @pytest.mark.asyncio
    async def test_execute_order_with_validation_failure(
        self, config, mock_executors, mock_managers
    ):
    ...
    @pytest.mark.asyncio
    async def test_execute_order_with_valid_payload(
        self, config, mock_executors, mock_managers
    ):
