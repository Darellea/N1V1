...
from data.dataset_versioning import DatasetVersionManager, MetadataError, migrate_legacy_dataset
...
class TestDataOptimizations(unittest.TestCase):
    ...
    def test_concatenation_generator_optimization(self):
        ...
        for i in range(150):  # Large dataset (> 100 DataFrames)
            timestamps = pd.date_range(base_time + pd.Timedelta(hours=i*24), periods=100, freq='h')
            data = {
...
class TestStructuredLogging(unittest.TestCase):
    ...
    async def test_data_fetcher_structured_logging(self):
        ...
        with patch('data.data_fetcher.logger') as mock_logger:
            ...
            with patch.object(self.fetcher, '_fetch_from_exchange', side_effect=mock_fetch):
