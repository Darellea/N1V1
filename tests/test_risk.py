...
async def test_initialization(risk_manager, risk_config):
    ...
    assert risk_manager.max_position_size == Decimal(str(risk_config['max_position_size']))
    assert risk_manager.max_daily_loss == Decimal(str(risk_config['max_daily_drawdown']))
    assert risk_manager.risk_reward_ratio == Decimal(str(risk_config['risk_reward_ratio']))
...
async def test_fixed_fractional_position_sizing(risk_manager, long_signal):
    ...
    expected_size = Decimal('10000') * Decimal('0.1') / Decimal('0.05')  # 5% risk (100->95)
...
async def test_volatility_position_sizing(risk_manager, long_signal):
    ...
    # Mock market data
    mock_data = {
        'close': pd.Series([90, 91, 92, 93, 94, 95, 96, 97, 98, 99, 100])
    }
    ...
    position_size = await risk_manager.calculate_position_size(long_signal, {'close': pd.Series([100])})
...
async def test_max_concurrent_trades(risk_manager, long_signal):
    ...
    risk_manager._get_current_positions = AsyncMock(return_value=['BTC/USDT', 'ETH/USDT', 'SOL/USDT'])
