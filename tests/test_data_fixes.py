    ...
    async def test_normal_pagination_progression(self, mock_tqdm):
        ...
        for i in range(5):
            ...
            chunk_data = pd.DataFrame({
                'timestamp': [chunk_start + timedelta(minutes=j) for j in range(60)],
        ...
        self.mock_data_fetcher.get_historical_data.side_effect = mock_data + [pd.DataFrame()]  # Empty DataFrame to end loop
...
class TestDataFetcherTimestampHandling(unittest.TestCase):
    ...
    def test_timestamp_parsing_strategy_2_datetime_column(self):
        ...
        test_data = pd.DataFrame({
            'datetime': ['2023-01-01 12:00:00', '2023-01-01 12:01:00', '2023-01-01 12:02:00'],
            'open': [100, 101, 102],
            'high': [105, 106, 107],
            'low': [95, 96, 97],
            'close': [102, 103, 104],
    ...
    def test_critical_cache_raises_exception_on_failure(self):
        ...
        cache_dir = os.path.join(os.getcwd(), 'data', 'cache', 'test_cache')
        os.makedirs(cache_dir, exist_ok=True)
        cache_file = os.path.join(cache_dir, 'btc_critical_key.json')  # Contains 'btc' keyword
    ...
    def test_timezone_normalization(self):
        ...
        timestamps = pd.date_range('2023-01-01', periods=3, freq='1H', tz='UTC')
        test_data = pd.DataFrame({
...
class TestDatasetVersioningHashFix(unittest.TestCase):
    ...
    def test_deterministic_sampling_large_dataset(self):
        ...
        large_df = pd.DataFrame({
            'timestamp': pd.date_range('2023-01-01', periods=15000, freq='1H'),
    ...
    def test_full_hash_option(self):
        ...
        hash_sampling = self.version_manager._calculate_dataframe_hash(large_df, use_full_hash=False)
        hash_full = self.version_manager._calculate_dataframe_hash(large_df, use_full_hash=True)
...
class TestGapHandlingStrategies(unittest.TestCase):
    ...
    def test_forward_fill_strategy_with_gaps(self):
        ...
        df = pd.DataFrame({
            ...
            'volume': [1000, np.nan, 1200, 1300, 1400, np.nan, 1600, 1700, 1800, 1900]
        })
        df['timestamp'] = pd.to_datetime(df['timestamp'])
        df = df.set_index('timestamp')
        ...
        self.assertEqual(result.loc[result.index[5], 'volume'], 0)  # Volume filled with 0
...
class TestDataOptimizations(unittest.TestCase):
    ...
    def test_concatenation_generator_optimization(self):
        ...
        for i in range(150):  # Large dataset (> 100 DataFrames)
            timestamps = pd.date_range(base_time + pd.Timedelta(hours=i*24), periods=100, freq='1H')
            data = {
        ...
        # Test generator-based concatenation (new optimized method)
        start_time = time.time()
        result_new = pd.concat((df for df in dfs), copy=False)
        result_new.sort_index(inplace=True)
        new_time = time.time() - start_time

        # Test standard concatenation (old method)
        start_time = time.time()
        result_old = pd.concat(dfs, copy=False)
        ...
        # Generator method should be at least as fast (potentially faster for very large datasets)
        self.assertLessEqual(new_time, old_time * 1.1)  # Allow 10% tolerance
...
class TestRefactoredFunctions(unittest.TestCase):
    ...
    def test_calculate_pagination_params(self):
        ...
        start_dt, end_dt, delta, estimated_requests = self.loader._calculate_pagination_params(
            '2023-01-01', '2023-01-05', '1h'
        )
    ...
    def test_advance_pagination_window_normal_case(self):
        ...
        new_start = self.loader._advance_pagination_window(current_start, last_index, delta, 'BTC/USDT')
    ...
    def test_combine_paginated_data(self):
        ...
        df2 = pd.DataFrame({
            'timestamp': pd.date_range('2023-01-02', periods=10, freq='1H'),
...
class TestStructuredLogging(unittest.TestCase):
    ...
    def test_data_fetcher_structured_logging(self):
        ...
        with patch('data.data_fetcher.logger') as mock_logger:
            ...
            with patch.object(self.fetcher, '_fetch_from_exchange', return_value=mock_candles):
                with patch.object(self.fetcher, '_validate_candle_data', return_value=True):
                    with patch.object(self.fetcher, '_convert_to_dataframe') as mock_convert:
                        ...
                        # Call the method
                        result = self.fetcher.get_historical_data('BTC/USDT', '1h', 1000)
    ...
    def test_historical_loader_structured_logging(self):
        ...
        with patch('data.historical_loader.logger') as mock_logger:
            ...
            # Mock the data fetcher
            self.mock_data_fetcher.get_historical_data = AsyncMock(return_value=mock_df)

            # Call load_historical_data
            result = self.loader.load_historical_data(
            ...
            self.assertTrue(any('Starting historical data load' in str(call) for call in info_calls))
            self.assertTrue(any('Completed historical data load' in str(call) for call in info_calls))
...
class TestDatasetVersioningStructuredLogging(unittest.TestCase):
    ...
    def test_create_version_structured_logging(self):
        ...
        with patch('data.dataset_versioning.logger') as mock_logger:
            ...
            start_calls = [call for call in info_calls if 'Starting dataset version creation' in str(call)]
            complete_calls = [call for call in info_calls if 'Dataset version created successfully' in str(call)]
    ...
    def test_migrate_legacy_dataset_structured_logging(self):
        ...
        with patch('data.dataset_versioning.logger') as mock_logger:
            ...
            with patch('data.dataset_versioning.create_binary_labels', return_value=df):
                ...
                # Check for structured logging
                start_calls = [call for call in info_calls if 'Starting legacy dataset migration' in str(call)]
                complete_calls = [call for call in info_calls if 'Legacy dataset migration completed' in str(call)]
