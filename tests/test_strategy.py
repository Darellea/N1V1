...
async def test_indicator_calculation(concrete_strategy, sample_ohlcv_data):
    ...
    data_with_indicators = await concrete_strategy.calculate_indicators(sample_ohlcv_data)
    assert 'sma_5' in data_with_indicators.columns
    assert 'sma_10' in data_with_indicators.columns
    assert not data_with_indicators['sma_5'].iloc[:4].notna().any()  # First 4 should be NaN
    assert data_with_indicators['sma_5'].iloc[4:].notna().all()  # After 5 should have values
...
async def test_strategy_error_handling(concrete_strategy, mock_data_fetcher):
    ...
    with patch.object(concrete_strategy, 'calculate_indicators', side_effect=Exception("Test error")):
        signals = await concrete_strategy.run()
        assert len(signals) == 0  # Should return empty list on error

    # Force an error in signal generation
    with patch.object(concrete_strategy, 'generate_signals', side_effect=Exception("Test error")):
        signals = await concrete_strategy.run()
        assert len(signals) == 0  # Should return empty list on error
