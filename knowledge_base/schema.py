...
class MarketCondition:
    ...
    def __eq__(self, other):
        ...
        return (
            ...
            and self.trend_strength == other.trend_strength
            and self.liquidity_score == other.liquidity_score
            and self.volume_trend == other.volume_trend
            and self.price_range == other.price_range
        )
...
class KnowledgeEntry:
    ...
    def update_performance(self, new_performance: PerformanceMetrics):
        ...
        self.performance.max_drawdown = max(
            self.performance.max_drawdown, new_performance.max_drawdown
        )
        self.performance.avg_win = (
            self.performance.avg_win * weight_old + new_performance.avg_win * weight_new
        )
    ...
    def _recalculate_confidence(self):
        ...
        sample_confidence = min(
            1.0, self.sample_size / 50
        )  # 50 samples for max confidence
...
class KnowledgeQuery:
    ...
    limit: int = 10
    sort_by: str = (
        "confidence_score"  # confidence_score, win_rate, profit_factor, sample_size
    )
...
def validate_knowledge_entry(entry: KnowledgeEntry) -> List[str]:
    ...
    if entry.confidence_score < 0 or entry.confidence_score > 1:
        errors.append(
            f"Confidence score {entry.confidence_score} must be between 0 and 1"
        )
    ...
    if entry.performance.total_trades < 0:
        errors.append(
            f"Total trades {entry.performance.total_trades} cannot be negative"
        )
