...
class ValidationAssetsConfig:
    ...
    validation_assets: List[Dict[str, Any]] = field(
...
class CrossAssetValidationConfig:
    ...
    # Core components
    validation_criteria: ValidationCriteriaConfig = field(
        default_factory=ValidationCriteriaConfig
    )
    asset_selector: ValidationAssetsConfig = field(
        default_factory=ValidationAssetsConfig
    )
...
class BayesianOptimizationConfig:
    ...
    acquisition_function: str = (
        "expected_improvement"  # 'expected_improvement', 'upper_confidence_bound'
    )
...
class IndicatorParametersConfig:
    ...
    bollinger_bands: Dict[str, Any] = field(
        default_factory=lambda: {
    ...
    moving_average: Dict[str, Any] = field(
        default_factory=lambda: {
    ...
    atr: Dict[str, Any] = field(
        default_factory=lambda: {"period": {"min": 5, "max": 30, "default": 14}}
    )

    volume: Dict[str, Any] = field(
        default_factory=lambda: {"period": {"min": 5, "max": 50, "default": 20}}
    )

    price_action: Dict[str, Any] = field(
        default_factory=lambda: {"lookback": {"min": 3, "max": 20, "default": 5}}
    )
...
class RiskManagementParametersConfig:
    ...
    stop_loss: Dict[str, Any] = field(
        default_factory=lambda: {"min": 0.005, "max": 0.1, "default": 0.02}
    )

    take_profit: Dict[str, Any] = field(
        default_factory=lambda: {"min": 0.01, "max": 0.2, "default": 0.05}
    )

    position_sizing: Dict[str, Any] = field(
        default_factory=lambda: {
            "method": {
                "allowed_values": ["fixed", "percentage", "kelly"],
                "default": "percentage",
            },
...
class OptimizationConfig:
    ...
    cross_asset_validation: CrossAssetValidationConfig = field(
        default_factory=CrossAssetValidationConfig
    )
    strategy_generation: StrategyGenerationConfig = field(
        default_factory=StrategyGenerationConfig
    )
    bayesian_optimization: BayesianOptimizationConfig = field(
        default_factory=BayesianOptimizationConfig
    )
    ...
    indicators: IndicatorParametersConfig = field(
        default_factory=IndicatorParametersConfig
    )
    signal_logic: SignalLogicParametersConfig = field(
        default_factory=SignalLogicParametersConfig
    )
    risk_management: RiskManagementParametersConfig = field(
        default_factory=RiskManagementParametersConfig
    )
    ...
    def validate(self) -> List[str]:
        ...
        if not (
            0 < self.cross_asset_validation.validation_criteria.required_pass_rate <= 1
        ):
