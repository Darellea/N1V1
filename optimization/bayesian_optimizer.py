...
class BayesianOptimizer:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        if not SKLEARN_AVAILABLE:
            logger.warning("Scikit-learn not available, Bayesian optimization will be disabled")
            self.enabled = False
            return
        ...
        self.min_observations_for_training = config.get('min_observations_for_training', 5)
        ...
        if self.enabled:
            ...
            self.logger.info("BayesianOptimizer initialized with Gaussian Process model")
        else:
            self.logger.info("BayesianOptimizer initialized but disabled (scikit-learn not available)")
    ...
    def _initialize_gp_model(self) -> None:
        ...
        if self.gp_kernel_type == 'rbf':
            ...
            kernel = gp.kernels.ConstantKernel(1.0) * gp.kernels.RBF(1.0)
            self.logger.warning(f"Unknown kernel type '{self.gp_kernel_type}', using RBF")
    ...
    def add_observation(self, genome: StrategyGenome, fitness: float) -> None:
        ...
        self.logger.debug(f"Added observation: fitness={fitness:.4f}, features_shape={features.shape}")

    def suggest_next_genome(self, population: List[StrategyGenome]) -> Optional[BayesianOptimizationResult]:
        """
        ...
        """
        if not self.enabled or len(self.observations) < self.min_observations_for_training:
            return None
        ...
        try:
            ...
            for candidate in candidates:
                acquisition_value, metadata = self._evaluate_acquisition_function(candidate)
            ...
            if best_candidate:
                ...
                self.logger.info(f"Bayesian optimization suggested genome with acquisition value: {best_acquisition_value:.4f}")
                return result
    ...
    def _train_model(self) -> None:
        ...
        try:
            ...
            self.logger.info(f"Trained GP model with {len(self.observations)} observations")

        except Exception as e:
            self.logger.error(f"Failed to train GP model: {str(e)}")
            self.is_trained = False

    def _generate_candidates(self, population: List[StrategyGenome]) -> List[StrategyGenome]:
    ...
    def _evaluate_acquisition_function(self, genome: StrategyGenome) -> Tuple[float, Dict[str, Any]]:
        """
        ...
        try:
            ...
            if self.acquisition_function == 'expected_improvement':
                acquisition_value, metadata = self._expected_improvement(y_pred[0], y_std[0], best_fitness)
            elif self.acquisition_function == 'upper_confidence_bound':
                acquisition_value, metadata = self._upper_confidence_bound(y_pred[0], y_std[0])
            elif self.acquisition_function == 'probability_of_improvement':
                acquisition_value, metadata = self._probability_of_improvement(y_pred[0], y_std[0], best_fitness)
            else:
    ...
    def _expected_improvement(self, mean: float, std: float, best_fitness: float) -> Tuple[float, Dict[str, Any]]:
        """Calculate Expected Improvement acquisition function."""
        improvement = mean - best_fitness

        if std > 0:
            z = improvement / std
            ei = improvement * self._normal_cdf(z) + std * self._normal_pdf(z) * self.ei_xi
            ei = max(0, ei)
        else:
    ...
    def _upper_confidence_bound(self, mean: float, std: float) -> Tuple[float, Dict[str, Any]]:
    ...
    def _probability_of_improvement(self, mean: float, std: float, best_fitness: float) -> Tuple[float, Dict[str, Any]]:
    ...
    def __str__(self) -> str:
        ...
        return (f"BayesianOptimizer(observations={status['num_observations']}, "
...
def get_bayesian_optimizer(config: Optional[Dict[str, Any]] = None) -> BayesianOptimizer:
    """
