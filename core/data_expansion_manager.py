...
class DataExpansionManager:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.volatility_periods = config.get('volatility_periods', [
    ...
    async def collect_multi_pair_data(self, target_samples: int = 5000) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for pair in self.target_pairs:
                for timeframe in self.timeframes:
                    task = self._collect_pair_timeframe_data(pair, timeframe, target_samples)
                    tasks.append(task)
            ...
            for i, result in enumerate(collection_results):
                ...
                if isinstance(result, Exception):
                    ...
                    results['total_samples_collected'] += result.get('samples_collected', 0)
    ...
    async def _collect_pair_timeframe_data(self, pair: str, timeframe: str,
        ...
        try:
            ...
            for source in self.data_sources:
                source_data = await self._collect_from_source(source, pair, timeframe, target_samples)
            ...
            if len(combined_data) < self.min_samples_per_pair:
                logger.warning(f"Insufficient data for {pair} {timeframe}: {len(combined_data)} < {self.min_samples_per_pair}")
                return result

            # Validate and enhance data
            validated_data = self._validate_and_enhance_data(combined_data, pair, timeframe)
            ...
            logger.info(f"Collected {len(validated_data)} samples for {pair} {timeframe}")
    ...
    async def _collect_from_source(self, source_config: Dict[str, Any], pair: str,
                                 timeframe: str, target_samples: int) -> Optional[pd.DataFrame]:
        ...
        try:
            if source_type == 'csv':
                ...
                return await self._collect_from_api(source_config, pair, timeframe, target_samples)
            elif source_type == 'database':
                return await self._collect_from_database(source_config, pair, timeframe, target_samples)
            else:
    ...
    async def _collect_from_csv(self, source_config: Dict[str, Any], pair: str,
                              timeframe: str) -> Optional[pd.DataFrame]:
        """Collect data from CSV files."""
        file_pattern = source_config.get('file_pattern', f"{pair.replace('/', '_')}_{timeframe}.csv")
    ...
    async def _collect_from_api(self, source_config: Dict[str, Any], pair: str,
                              timeframe: str, target_samples: int) -> Optional[pd.DataFrame]:
        ...
        if 'start_time' in source_config:
            params['startTime'] = int(pd.Timestamp(source_config['start_time']).timestamp() * 1000)
        if 'end_time' in source_config:
            params['endTime'] = int(pd.Timestamp(source_config['end_time']).timestamp() * 1000)
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(url, headers=headers, params=params) as response:
                    if response.status == 200:
                        ...
                        df = self._convert_api_response_to_df(data, source_config.get('response_format', 'binance'))
                        return df
                    else:
    ...
    async def _collect_from_database(self, source_config: Dict[str, Any], pair: str,
                                   timeframe: str, target_samples: int) -> Optional[pd.DataFrame]:
    ...
    def _convert_api_response_to_df(self, data: List[Any], format_type: str) -> pd.DataFrame:
    ...
    def _validate_and_enhance_data(self, df: pd.DataFrame, pair: str, timeframe: str) -> pd.DataFrame:
    ...
    def _calculate_data_quality_score(self, df: pd.DataFrame) -> float:
        ...
        completeness = sum(1 for col in essential_cols if col in df.columns) / len(essential_cols)
        score *= completeness
    ...
    async def collect_volatility_period_data(self) -> Dict[str, Any]:
        ...
        for period in self.volatility_periods:
            try:
                ...
                results['total_samples_collected'] += period_data.get('samples_collected', 0)

                logger.info(f"Collected {period_data.get('samples_collected', 0)} samples for {period['name']}")
    ...
    async def _collect_single_volatility_period(self, period: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for pair in self.target_pairs:
            for timeframe in self.timeframes:
                try:
                    data = await self._collect_from_source(period_config, pair, timeframe, 5000)
                    if data is not None and not data.empty:
                        data['volatility_period'] = period_name
                        all_data.append(data)
                except Exception as e:
                    logger.warning(f"Error collecting {pair} {timeframe} for {period_name}: {e}")
    ...
    async def validate_data_integrity(self) -> Dict[str, Any]:
        ...
        for file_path in data_files:
            ...
            try:
                ...
                if df.empty:
                    validation_results['issues_found'].append(f"{file_path.name}: Empty file")
                ...
                if missing_cols:
                    validation_results['issues_found'].append(f"{file_path.name}: Missing columns {missing_cols}")
                    validation_results['invalid_files'] += 1
                    continue
                ...
                if quality_issues:
                    validation_results['issues_found'].extend([f"{file_path.name}: {issue}" for issue in quality_issues])
                    validation_results['invalid_files'] += 1
                else:
                ...
                validation_results['issues_found'].append(f"{file_path.name}: Error reading file - {str(e)}")
                validation_results['invalid_files'] += 1

        logger.info(f"Data integrity validation completed: {validation_results['valid_files']}/{validation_results['total_files_checked']} files valid")
...
async def run_data_collection(config: Dict[str, Any], target_samples: int = 5000) -> Dict[str, Any]:
    ...
    combined_results = {
        ...
        'total_samples': results['total_samples_collected'] + volatility_results['total_samples_collected']
    }
