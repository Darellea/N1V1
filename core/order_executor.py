...
class OrderExecutor:
        ...
        for signal in approved_signals:
            try:
                ...
                if not await self._validate_signal_security(signal):
                    log_security_event(
                        "signal_security_validation_failed",
                        {
                ...
                logger.exception(
                    f"Error executing order for signal {signal}: {sanitized_error}"
                )
                # Continue with other signals
    ...
    async def _execute_single_order(self, signal) -> None:
        ...
        try:
            ...
            if order_result:
                ...
                logger.info(
                    f"Order executed successfully: {order_result.get('id', 'unknown')}"
                )
            else:
                self.execution_stats["failed_orders"] += 1
                logger.warning(
                    f"Order execution failed after retries for signal: {signal}"
                )
    ...
    async def _execute_with_retry(self, signal) -> Optional[Dict[str, Any]]:
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if (
                    hasattr(self.order_manager, "reliability_manager")
                    and self.order_manager.reliability_manager
                ):
                    if self.order_manager.reliability_manager.safe_mode_active:
                        ctx_logger.warning(
                            "Circuit breaker active, skipping order execution"
                        )
                        return None
                ...
                if order_result:
                    ctx_logger.info(
                        f"Order executed successfully on attempt {attempt + 1}"
                    )
                    return order_result
                else:
                    ctx_logger.warning(
                        f"Order execution returned None on attempt {attempt + 1}"
                    )
                ...
                if attempt == max_retries:
                    ctx_logger.error(
                        f"Order execution failed after {max_retries + 1} attempts"
                    )
    ...
    def set_risk_parameters(self, risk_params: Dict[str, Any]):
        ...
        try:
            ...
            if hasattr(self.order_manager, "set_risk_parameters"):
                ...
                logger.warning(
                    "OrderManager does not support dynamic risk parameter updates"
                )
        except Exception as e:
        ...
        try:
            ...
            if not validator.register_order(signal_data):
                raise SecurityViolationException(
                    "Failed to register order for tracking"
                )
    ...
    async def _check_rate_limits(self, signal_data: Dict[str, Any]) -> bool:
        ...
        self._rate_limit_cache[symbol] = [
            ts for ts in self._rate_limit_cache[symbol] if current_time - ts < 60
        ]

        # Check rate limit (configurable, default 10 orders per minute per symbol)
        max_orders_per_minute = self.config.get("security", {}).get(
    ...
    def _validate_signal_integrity(self, signal) -> bool:
        ...
        try:
            ...
            log_security_event(
                "signal_integrity_check_failed",
                {"signal_id": getattr(signal, "id", "unknown"), "error": str(e)},
                "WARNING",
            )
            return False
    ...
    async def perform_security_health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            health_status["signal_validation"] = callable(
                getattr(self, "_validate_signal_security", None)
            )

            # Check schema validation
            health_status["schema_validation"] = callable(
                getattr(validator, "validate_order_schema", None)
            )

        except Exception as e:
            log_security_event(
                "security_health_check_failed",
                {"error": str(e), "component": "OrderExecutor"},
                "ERROR",
            )
