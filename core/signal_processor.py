...
class SignalProcessor:
    ...
    def __init__(
        self, config: Dict[str, Any], risk_manager=None, strategy_selector=None
    ):
    ...
    async def _generate_signals_with_selector(
        ...
        try:
            selected_strategy_class = self.strategy_selector.select_strategy(
            ...
            if selected_strategy_class:
                ...
                if selected_strategy:
                    ...
                    multi_tf_data = self._extract_multi_tf_data(
                        market_data, primary_symbol
                    )
            ...
            logger.error(
                f"Strategy selection error - configuration or runtime issue: {e}"
            )
            signals = await self._generate_signals_from_all_strategies(market_data)
    ...
    async def _generate_signals_from_all_strategies(
        ...
        for strategy in self.strategies:
            try:
                ...
                logger.error(
                    f"Signal generation error - invalid data or configuration in {strategy.__class__.__name__}: {e}"
                )
                # Continue with other strategies
            except asyncio.TimeoutError as e:
                logger.error(
                    f"Signal generation timeout in {strategy.__class__.__name__}: {e}"
                )
                # Continue with other strategies
            except Exception as e:
                logger.exception(
                    f"Unexpected error generating signals from {strategy.__class__.__name__}: {e}"
                )
                # Continue with other strategies
    ...
    async def evaluate_risk(
        ...
        for signal in signals:
            try:
                ...
                logger.error(
                    f"Risk evaluation error - invalid signal data or configuration: {e}"
                )
                ...
                logger.exception(
                    f"Unexpected error evaluating signal through risk manager: {e}"
                )
                # Conservatively reject signal on error

        logger.debug(
            f"Risk evaluation: {len(approved_signals)}/{len(signals)} signals approved"
        )
    ...
    def get_signal_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "strategy_selector_enabled": self.strategy_selector.enabled
    ...
    async def initialize_strategies(self, data_fetcher):
        ...
        for strategy in self.strategies:
            try:
                ...
                logger.error(
                    f"Strategy initialization error - invalid data fetcher or configuration in {strategy.__class__.__name__}: {e}"
                )
            except asyncio.TimeoutError as e:
                logger.error(
                    f"Strategy initialization timeout in {strategy.__class__.__name__}: {e}"
                )
            except Exception as e:
                logger.exception(
                    f"Unexpected error initializing strategy {strategy.__class__.__name__}: {e}"
                )
    ...
    async def process_signal(self, signal: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"processed": False, "error": str(e), "status": "failed"}
    ...
    async def shutdown_strategies(self):
        ...
        for strategy in self.strategies:
            try:
                ...
                logger.error(
                    f"Strategy shutdown error - invalid strategy state in {strategy.__class__.__name__}: {e}"
                )
            except asyncio.TimeoutError as e:
                logger.error(
                    f"Strategy shutdown timeout in {strategy.__class__.__name__}: {e}"
                )
            except Exception as e:
                logger.exception(
                    f"Unexpected error shutting down strategy {strategy.__class__.__name__}: {e}"
                )
