...
class SignalValidator:
        ...
        try:
            ...
            if signal.signal_type in [SignalType.ENTRY_LONG, SignalType.ENTRY_SHORT]:
                ...
                if signal.stop_loss and signal.current_price:
                    stop_distance = abs(signal.current_price - signal.stop_loss) / signal.current_price
                    min_stop_distance = getattr(self.risk_manager, "min_stop_distance", 0.005)  # 0.5% default

                    if stop_distance < min_stop_distance:
                        logger.debug(f"Stop loss distance too small: {stop_distance:.4%} < {min_stop_distance:.4%}")
                        return False

            # Check take profit requirements
            if signal.take_profit and signal.current_price:
                tp_distance = abs(signal.take_profit - signal.current_price) / signal.current_price
                min_tp_distance = getattr(self.risk_manager, "min_take_profit_distance", 0.01)  # 1% default

                if tp_distance < min_tp_distance:
                    logger.debug(f"Take profit distance too small: {tp_distance:.4%} < {min_tp_distance:.4%}")
                    return False
    ...
    def validate_signal_confidence(self, signal: TradingSignal, min_confidence: float = 0.5) -> bool:
        """
        ...
        try:
            ...
            if confidence < min_confidence:
                logger.debug(f"Signal confidence too low: {confidence:.3f} < {min_confidence:.3f}")
                return False
    ...
    def check_for_duplicates(self, signal: TradingSignal, recent_signals: list,
        ...
        try:
            ...
            for recent_signal in recent_signals:
                ...
                if (recent_signal.symbol == signal.symbol and
                    recent_signal.strategy_id == signal.strategy_id and
                    recent_signal.signal_type == signal.signal_type):

                    # Check amount similarity (within 10%)
                    amount_diff = abs(recent_signal.amount - signal.amount) / recent_signal.amount
                    if amount_diff < 0.1:
    ...
    def perform_sanity_checks(self, signal: TradingSignal) -> bool:
        ...
        try:
            ...
            if signal.stop_loss and signal.current_price:
                loss_pct = abs(signal.stop_loss - signal.current_price) / signal.current_price
