...
from ml.trainer import train_model_binary, train_ensemble_model, create_ensemble_model
from ml.trainer import generate_enhanced_features
...
def validate_f1_score_target():
    ...
    exclude_cols = ['timestamp', 'open', 'high', 'low', 'close', 'volume',
                   'target', 'future_price', 'forward_return', 'sample_weight']
    feature_cols = [col for col in df_labeled.columns
                   if col not in exclude_cols and not col.startswith('DM_') and not col.startswith('TR')]
...
def validate_deterministic_reproduction():
    ...
    for run in [1, 2]:
        ...
        # Select features
        exclude_cols = ['timestamp', 'open', 'high', 'low', 'close', 'volume',
                       'target', 'future_price', 'forward_return']
        feature_cols = [col for col in df_labeled.columns
                       if col not in exclude_cols and not col.startswith('DM_') and not col.startswith('TR')]
...
def validate_dataset_versioning():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert loaded_metadata['description'] == 'Validation test dataset', 'Metadata mismatch'
