...
class ATRBreakoutStrategy(BaseStrategy, SignalGenerationMixin):
    ...
    async def calculate_indicators(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if "symbol" in data.columns and data["symbol"].nunique() > 1:
            ...
            data = grouped.apply(calculate_atr_indicators).reset_index(
                level=0, drop=True
            )
        else:
            # Single symbol data
    ...
    async def _generate_signals_for_symbol(
        ...
        try:
            ...
            breakout_up = (
                last_row["high"] > last_row["breakout_upper"]
                and prev_row["high"] <= prev_row["breakout_upper"]
            )

            # Check for breakout below lower threshold
            breakout_down = (
                last_row["low"] < last_row["breakout_lower"]
                and prev_row["low"] >= prev_row["breakout_lower"]
            )
                ...
                try:
                    ...
                    if not pd.isna(trend_slope):
                        ...
                        if breakout_up and trend_slope < 0:
                            trend_confirmed = (
                                False  # Strong downtrend against long breakout
                            )
                        elif breakout_down and trend_slope > 0:
                            trend_confirmed = (
                                False  # Strong uptrend against short breakout
                            )
            ...
            if breakout_up:
                ...
                if not data_with_atr.empty and isinstance(
                    data_with_atr.index, pd.DatetimeIndex
                ):
