...
class EMACrossStrategy(BaseStrategy):
    ...
    async def calculate_indicators(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        def calculate_emas(group):
            group['fast_ema'] = group['close'].ewm(span=self.params['fast_ema'], adjust=False).mean()
            group['slow_ema'] = group['close'].ewm(span=self.params['slow_ema'], adjust=False).mean()
            return group
    ...
    async def generate_signals(self, data: pd.DataFrame) -> List[TradingSignal]:
        ...
        for symbol, group in grouped:
            ...
            if (prev_row['fast_ema'] <= prev_row['slow_ema'] and 
                last_row['fast_ema'] > last_row['slow_ema']):
            ...
            elif (prev_row['fast_ema'] >= prev_row['slow_ema'] and 
                  last_row['fast_ema'] < last_row['slow_ema']):
                signals.append(self.create_signal(
                    symbol=symbol,
