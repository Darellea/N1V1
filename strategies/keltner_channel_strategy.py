    ...
    async def calculate_indicators(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.params["squeeze_filter"]:
            ...
            avg_width = (
                ...
                .rolling(window=int(self.params["sma_period"]))
                .mean()
            )
        ...
        try:
            ...
            for symbol in data["symbol"].unique():
                ...
                if pd.isna(last_row["keltner_position"]):
                    ...
                    logger.warning(
                        f"NaN detected in Keltner position for symbol {symbol} during vectorized processing. "
                ...
                if (
                    self.params["squeeze_filter"]
                    and "squeeze_ratio" in last_row.index
                    and not pd.isna(last_row["squeeze_ratio"])
                ):
                    try:
                ...
                breakout_up = (
                    last_row["high"] > last_row["keltner_upper"]
                    and prev_row["high"] <= prev_row["keltner_upper"]
                )

                # Check for breakout below lower channel
                breakout_down = (
                    last_row["low"] < last_row["keltner_lower"]
                    and prev_row["low"] >= prev_row["keltner_lower"]
                )
                ...
                if breakout_up:
                    ...
                    signals.append(
                        self.create_signal(
                            ...
                            order_type="market",
                            amount=self.params["position_size"]
                            * 0.8,  # Smaller position for reversion
                            current_price=current_price,
                            stop_loss=current_price
                            * (1 - self.params["stop_loss_pct"]),
                            take_profit=current_price
                            * (
                                1 + self.params["take_profit_pct"] * 0.6
                            ),  # Smaller target
                            metadata={
    ...
    async def _generate_signals_for_symbol(
        ...
        try:
            ...
            if self.params["volume_filter"] and "volume" in last_row.index:
                try:
                    ...
                    if len(data_with_keltner) >= volume_period:
                        avg_volume = (
                            data_with_keltner["volume"].tail(volume_period).mean()
                        )
                        current_volume = last_row["volume"]
                        volume_confirmed = current_volume >= (
                            avg_volume * self.params["volume_threshold"]
            ...
            if pd.isna(last_row["keltner_position"]):
                logger = logging.getLogger(__name__)
                logger.warning(
                    f"NaN detected in Keltner position for symbol {symbol}. Skipping signal generation. "
            ...
            if breakout_up:
                ...
                # Extract deterministic timestamp from the data that triggered the signal
                signal_timestamp = None
                if not data_with_keltner.empty and isinstance(
                    data_with_keltner.index, pd.DatetimeIndex
                ):
                ...
                signals.append(
                    self.create_signal(
                        ...
                        * 0.8,  # Smaller position for reversion
                        ...
                        stop_loss=current_price * (1 - self.params["stop_loss_pct"]),
                        take_profit=current_price
                        * (1 + self.params["take_profit_pct"] * 0.6),  # Smaller target
                        metadata={
                            "signal_type": "keltner_reversion",
                            "reversion_type": "oversold",
                            "keltner_middle": last_row["keltner_middle"],
