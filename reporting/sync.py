...
class DashboardSync:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.streamlit_data_dir = self.config.get("streamlit", {}).get(
    ...
    def _get_default_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "streamlit": {"enabled": True, "data_dir": "dashboard/data"},
        }
    ...
    def _sync_to_grafana(self, result: MetricsResult) -> bool:
        ...
        try:
            ...
            response = requests.post(
                url, json=prometheus_data, headers=headers, timeout=self.timeout
            )

            if response.status_code in [200, 202]:
                self.logger.info(
                    f"Successfully synced metrics to Grafana for {result.strategy_id}"
                )
                return True
            else:
                self.logger.error(
                    f"Grafana sync failed with status {response.status_code}: {response.text}"
                )
                return False
    ...
    def _sync_to_streamlit(self, result: MetricsResult) -> bool:
        ...
        try:
            ...
            strategy_file = os.path.join(
                self.streamlit_data_dir, f"metrics_{result.strategy_id}.json"
            )
            with open(strategy_file, "w") as f:
                json.dump(result.to_dict(), f, indent=2, default=str)

            self.logger.info(
                f"Successfully synced metrics to Streamlit for {result.strategy_id}"
            )
            return True
    ...
    def get_dashboard_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "grafana": {
                ...
                "connected": self._test_grafana_connection()
                if self.grafana_enabled
                else False,
            },
            "streamlit": {
                "enabled": self.streamlit_enabled,
                "data_dir_exists": os.path.exists(self.streamlit_data_dir)
                if self.streamlit_enabled
                else False,
            },
        }
    ...
    def _test_grafana_connection(self) -> bool:
        ...
        try:
            ...
            response = requests.get(
                f"{self.grafana_url}/api/health", headers=headers, timeout=self.timeout
            )
    ...
    def get_all_strategies(self) -> List[str]:
        ...
        try:
            ...
            for filename in os.listdir(self.data_dir):
                if (
                    filename.startswith("metrics_")
                    and filename.endswith(".json")
                    and not filename.startswith("metrics_history")
                ):
