...
class ModelRegistry:
    ...
    def register_model(
        ...
        content_hash = hashlib.md5(
            f"{model_path}{config_path}{data_version}".encode()
        ).hexdigest()[:8]
    ...
    def deploy_canary(self, model_name: str, version_id: str, percentage: float):
        ...
        self._save_registry()
        logger.info(
            f"Deployed {model_name} version {version_id} as canary ({percentage*100:.1f}%)"
        )
    ...
    def rollback_model(self, model_name: str, target_version_id: Optional[str] = None):
        ...
        if target_version_id is None:
            ...
            target_version_id = max(
                model_versions, key=lambda v: v.created_at
            ).version_id
    ...
    def validate_model_performance(
        self, version_id: str, validation_metrics: Dict[str, float]
    ) -> bool:
        ...
        thresholds = {"auc": 0.5, "sharpe_ratio": 0.0, "max_drawdown": -0.15}  # -15%
            ...
            if metric in validation_metrics:
                if metric == "max_drawdown":
                    ...
                    if validation_metrics[metric] < threshold:
                        ...
                        logger.warning(
                            f"Model {version_id} failed {metric} validation: {validation_metrics[metric]} < {threshold}"
                        )
                        break
                else:
...
class RetrainingScheduler:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.model_registry = ModelRegistry(
            config.get("registry_path", "models/registry")
        )
    ...
    def _schedule_job(self, job: RetrainingJob):
        ...
        if job.schedule_type == "daily":
            ...
            self.scheduler.every().monday.at("02:00").do(
                self._run_job_async, job.job_id
            )
    ...
    async def _execute_retraining_job(self, job_id: str):
        ...
        try:
            ...
            if result["success"]:
                ...
                job.status = "failed"
                logger.error(
                    f"Retraining job {job_id} failed: {result.get('error', 'Unknown error')}"
                )
    ...
    async def _run_retraining_pipeline(self, job: RetrainingJob) -> Dict[str, Any]:
        ...
        try:
            ...
            training_data = await self._prepare_training_data(
                data_collection_result["data_path"]
            )
            ...
            validation_result = await self._validate_new_model(
                training_result["model_path"], training_result["config_path"]
            )
            if not validation_result["success"]:
                return validation_result

            # Step 6: Register and deploy model
            deployment_result = await self._deploy_new_model(
                job, training_result, validation_result
            )
    ...
    async def _check_data_freshness(self, job: RetrainingJob) -> Dict[str, Any]:
        ...
        if not collection_summary_path.exists():
            return {"is_fresh": False, "message": "No data collection summary found"}
        ...
        try:
            ...
            return {"is_fresh": True, "hours_old": hours_since_collection}

        except Exception as e:
            logger.error(f"Error checking data freshness: {e}")
            return {"is_fresh": False, "message": f"Error checking data freshness: {e}"}
    ...
    async def _collect_training_data(self, job: RetrainingJob) -> Dict[str, Any]:
        ...
        try:
            ...
            if (
                results["total_samples_collected"] < target_samples * 0.5
    ...
    async def _prepare_training_data(self, data_path: str) -> pd.DataFrame:
        ...
        logger.warning(
            "No sample data found, creating synthetic data for demonstration"
        )
    ...
    async def _train_new_model(
        self, job: RetrainingJob, training_data: pd.DataFrame
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if not feature_columns:
                ...
                training_data = generate_enhanced_features(
                    training_data, include_multi_horizon=True
                )
                training_data = create_binary_labels(
                    training_data, horizon=5, profit_threshold=0.005
                )
                feature_columns = [
                    col
                    for col in training_data.columns
    ...
    async def _validate_new_model(
        ...
        try:
            ...
            thresholds = {"auc": 0.5, "sharpe_ratio": 0.5, "max_drawdown": -0.15}
    ...
    async def _deploy_new_model(
        ...
        try:
            ...
            if validation_passed:
                ...
                self.model_registry.deploy_canary(
                    job.target_model, version_id, canary_percentage
                )

                # Schedule full deployment after successful canary period
                asyncio.create_task(
                    self._schedule_full_deployment(job.target_model, version_id)
                )
                ...
                logger.warning(
                    f"Model {version_id} failed validation, keeping current version"
                )
                return {"status": "validation_failed", "version_id": version_id}
    ...
    async def _schedule_full_deployment(self, model_name: str, version_id: str):
        ...
        try:
            ...
            canary_metrics = await self._evaluate_canary_performance(
            ...
            if canary_metrics["success"]:
                ...
                logger.info(
                    f"Full deployment completed for {model_name} version {version_id}"
                )
            else:
                logger.warning(
                    f"Canary deployment failed for {model_name} version {version_id}"
                )
...
async def run_retraining_job(config: Dict[str, Any], job_id: str) -> Dict[str, Any]:
    ...
    return {"success": status["status"] == "completed", "job_status": status}
        ...
        "jobs": [
            {
                ...
                "performance_thresholds": {"auc": 0.5, "sharpe_ratio": 0.5},
            }
        ],
