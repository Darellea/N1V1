    ...
    def reset_level(self, logger_name: str) -> None:
        ...
        with self.lock:
            if logger_name in self.original_levels:
                ...
                logger.info(f"Reset log level for '{logger_name}' to {logging.getLevelName(original_level)}")
...
class LoggingManager:
    ...
    def _setup_root_logger(self) -> None:
        ...
        root_logger.setLevel(getattr(logging, self.config.get('level', 'INFO').upper(), logging.INFO))
    ...
    def _setup_component_loggers(self) -> None:
        ...
        components = [
            "trading", "risk", "execution", "data", "api", "database",
            "notifications", "monitoring", "optimization", "backtest"
        ]
    ...
    def create_operation_logger(self, component: str, operation: str,
...
def get_component_logger(component: str, operation: Optional[str] = None) -> ContextAdapter:
    """Get component logger (backward compatibility)."""
    return _logging_manager.get_component_logger(component, operation)


def create_operation_logger(component: str, operation: str, **context) -> ContextAdapter:
...
def log_error_with_context(logger: ContextAdapter, error: Exception,
...
def log_performance_metric(logger: ContextAdapter, metric_name: str,
                          value: Union[int, float], **context) -> None:
    """
...
def log_system_status(logger: ContextAdapter, status: str,
                     component: str, **context) -> None:
    """
