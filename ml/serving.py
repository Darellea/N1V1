...
from .model_loader import predict as local_predict
...
INFERENCE_LATENCY = Histogram(
    "inference_latency_seconds", "Inference latency in seconds", ["model_name"]
)
...
class PredictionRequest(BaseModel):
    ...
    features: Dict[str, List[float]] = Field(
        ..., description="Feature data as dict of lists"
    )
...
def load_model(model_name: str) -> Any:
    ...
    if model_name not in models:
        try:
            ...
            logger.info(
                f"Loaded model {model_name} version {model_versions[model_name]}"
            )
...
def process_single_prediction(request) -> Dict[str, Any]:
    ...
    try:
        ...
        INFERENCE_LATENCY.labels(model_name=request.model_name).observe(
            latency_ms / 1000
        )

        logger.info(
            f"Prediction completed for {request.model_name}, correlation_id: {correlation_id}, latency: {latency_ms:.2f}ms"
        )
        ...
        INFERENCE_REQUESTS.labels(
            model_name=getattr(request, "model_name", "unknown"), status="error"
        ).inc()
        logger.error(
            f"Prediction failed for {getattr(request, 'model_name', 'unknown')}, correlation_id: {correlation_id}: {e}"
        )
...
async def predict_batch(request: BatchPredictionRequest):
    ...
    tasks = [
        asyncio.get_event_loop().run_in_executor(
            executor, process_single_prediction, req
        )
