...
class FeatureExtractor:
    ...
    def extract_features(
        self, data: pd.DataFrame, fit_scaler: bool = True
    ) -> pd.DataFrame:
        """
        ...
        if not self.validate_ohlcv_data(data):
            raise ValueError(
                "Data must contain OHLCV columns: open, high, low, close, volume"
            )

        if len(data) < self.config["validation"]["require_min_rows"]:
            logger.warning(
                f"Insufficient data rows: {len(data)} < {self.config['validation']['require_min_rows']}"
            )
            return pd.DataFrame()
        ...
        try:
            ...
            logger.info(
                f"Extracted {len(df_scaled.columns)} features from {len(data)} data points"
            )
            return df_scaled
    ...
    def _calculate_indicators(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        self.feature_columns.extend(
            [col for col in indicator_names if col in df.columns]
        )
    ...
    def _add_volume_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        window_size = volume_config.get(
            "window_size", 20
        )  # Configurable window size for volume features

        if volume_config.get("volume_sma", True):
            df[f"volume_sma_{window_size}"] = (
                df["volume"].rolling(window=window_size).mean()
            )
        ...
        df["volume_ma_ratio"] = (
            df["volume"] / df["volume"].rolling(window=window_size).mean()
        )
    ...
    def _add_lagged_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        key_features = lagged_config.get(
            "key_features",
            ["close", "rsi", "ema", "macd", "bb_upper", "bb_lower", "atr", "adx"],
        )
    ...
    def _clean_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if handle_missing == "drop":
            ...
            if df.empty:
                logger.warning(
                    "DataFrame became empty after dropna, filling with zeros"
                )
                df = df.fillna(0)
    ...
    def _scale_features(
        ...
        try:
            ...
            if not self.is_fitted:
                raise RuntimeError(
                    "Scaler is not fitted. Please fit the scaler first by calling extract_features with fit_scaler=True or load a pre-fitted scaler."
                )
    ...
    def add_cross_asset_features(
        self, data: pd.DataFrame, asset_data: Dict[str, pd.DataFrame]
    ) -> pd.DataFrame:
        """
            ...
            if "close" in asset_df.columns and len(asset_df) == len(df):
                for window in window_sizes:
                    if len(df) > window:
                        ...
                        df[corr_col] = (
                            df["close"].rolling(window).corr(asset_df["close"])
                        )
    ...
    def add_time_anchored_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for period in momentum_periods:
            if len(df) > period:
                ...
                df[mom_col] = (df["close"] - df["close"].shift(period)) / df[
                    "close"
                ].shift(period)
    ...
    def get_drift_report(
        self, drift_results: Dict[str, Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        drifted_features = sum(
            1 for result in drift_results.values() if result["drift_detected"]
        )
        ...
        report = {
            ...
            "drift_summary": {
                "ks_based_drift": sum(
                    1 for r in drift_results.values() if r.get("ks_pvalue", 1.0) < 0.05
                ),
                "psi_based_drift": sum(
                    1 for r in drift_results.values() if r.get("psi_score", 0) > 0.25
                ),
            },
        }
    ...
    def _get_most_drifted_features(
        self, drift_results: Dict[str, Dict[str, Any]], top_n: int = 5
    ) -> List[Dict[str, Any]]:
        """
...
def batch_extract_features(
    data_dict: Dict[str, pd.DataFrame], config: Optional[Dict[str, Any]] = None
...
def generate_time_anchored_features(data: pd.DataFrame) -> pd.DataFrame:
    ...
    for period in momentum_periods:
        if len(data) > period:
            ...
            features_df[mom_col] = (data["close"] - data["close"].shift(period)) / data[
                "close"
            ].shift(period)
...
class TimeAnchoredFeatureGenerator:
    ...
    def generate_volume_profile_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if len(data) > period:
            ...
            features_df[vol_zscore_col] = (
                data["volume"] - features_df[vol_ma_col]
            ) / features_df[vol_std_col]
...
def detect_feature_drift(
    reference_data: pd.DataFrame, current_data: pd.DataFrame, method: str = "ks"
        ...
        for col in data.columns:
            if data[col].dtype in ["int64", "float64"]:
                ...
                if std_val > 0:
                    ...
                    if extreme_count > 0:
                        issues.append(
                            f"Column '{col}' has {extreme_count} extreme values"
                        )
                        is_valid = False
            ...
            for i in range(len(numeric_cols)):
                for j in range(i + 1, len(numeric_cols)):
                    ...
                    if corr > max_correlation:
                        issues.append(
                            f"High correlation ({corr:.2f}) between '{numeric_cols[i]}' and '{numeric_cols[j]}'"
                        )
                        is_valid = False
