...
def compute_trend_strength(
    series: pd.Series, period: Optional[int] = None
) -> pd.Series:
...
def load_data(path: str) -> pd.DataFrame:
    ...
    try:
        ...
        if nan_rows > 0:
            logger.warning(
                f"Removing {nan_rows} rows with NaN values in essential columns"
            )
...
def generate_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["EMA_20"] = (
        df["EMA_20"].fillna(df["Close"])
        if nan_defaults["ema_20"] is None
        else df["EMA_20"].fillna(nan_defaults["ema_20"])
    )
    df["ATR"] = (
        df["ATR"].fillna(df["High"] - df["Low"])
        if nan_defaults["atr"] is None
        else df["ATR"].fillna(nan_defaults["atr"])
    )
...
def generate_enhanced_features(
    ...
    if include_multi_horizon:
        for horizon in [1, 3, 5, 24]:
            if len(df) > horizon:
                ...
                df[f"mean_return_{horizon}"] = (
                    df["Close"].pct_change().rolling(window=horizon).mean()
                )
                df[f"skew_{horizon}"] = (
                    df["Close"].pct_change().rolling(window=horizon).skew()
                )
                df[f"kurtosis_{horizon}"] = (
                    df["Close"].pct_change().rolling(window=horizon).kurt()
                )
    ...
    if include_regime_features:
        ...
        df["BB_upper"] = (
            df["BB_middle"] + bb_std_dev * df["Close"].rolling(window=bb_period).std()
        )
        df["BB_lower"] = (
            df["BB_middle"] - bb_std_dev * df["Close"].rolling(window=bb_period).std()
        )
...
def compute_adx(df: pd.DataFrame, period: int = 14) -> pd.Series:
    ...
    df["DI_plus"] = 100 * (
        df["DM_plus"].ewm(span=period).mean() / df["TR"].ewm(span=period).mean()
    )
    df["DI_minus"] = 100 * (
        df["DM_minus"].ewm(span=period).mean() / df["TR"].ewm(span=period).mean()
    )
    ...
    for col in columns:
        ...
        if method == "iqr":
            ...
            try:
                ...
                logging.warning(
                    "IsolationForest not available, skipping outlier removal for column: "
                    + col
                )
...
def create_sample_weights(
    ...
    profit_col: str = "forward_return",
    method: str = "class_balance",
) -> np.ndarray:
    """
    ...
    if method == "class_balance":
        ...
        class_weights = create_sample_weights(
            df, label_col, profit_col, "class_balance"
        )
        profit_weights = create_sample_weights(
            df, label_col, profit_col, "profit_impact"
        )
...
def perform_feature_selection(
    ...
    if method == "gain_importance":
        ...
        logging.warning(
            f"Feature selection method '{method}' not available, using all features"
        )
        selected_features = list(X.columns)

    logging.info(
        f"Selected {len(selected_features)} features using {method}: {selected_features}"
    )
...
def create_binary_labels(
    ...
    # Use config values if parameters not provided
    if horizon is None:
        horizon = TRAINER_CONFIG["model_training"][
            "default_n_splits"
        ]  # Use n_splits as default horizon
    ...
    if len(df) <= horizon:
        raise ValueError(
            f"Insufficient data for horizon {horizon}. Need at least {horizon + 1} samples, got {len(df)}."
        )
    ...
    logging.info(
        f"Created binary labels: {df['label_binary'].sum()} trade signals out of {len(df)} total samples"
    )
    logging.info(
        f"Binary label distribution: {df['label_binary'].value_counts().to_dict()}"
    )
...
def validate_inputs(
    ...
    if len(valid_features) != len(feature_columns):
        ...
        logging.warning(
            f"Missing feature columns: {missing_features}. Using only valid features: {valid_features}"
        )
...
def prepare_data(
    df: pd.DataFrame, feature_columns: List[str], label_col: str, n_splits: int
) -> Tuple[pd.DataFrame, pd.Series, Optional[np.ndarray], int]:
    """
    ...
    if len(X) < n_splits * 2:
        ...
        logging.warning(
            f"Adjusting n_splits from {n_splits} to {adjusted_splits} due to small dataset size ({len(X)} samples)"
        )
...
def perform_hyperparameter_tuning(
    ...
    if tune and OPTUNA_AVAILABLE:
        logging.info(
            f"Performing Optuna hyperparameter optimization with {n_trials} trials..."
        )
...
def _train_single_fold(
    fold_data: Tuple[
        pd.DataFrame, pd.Series, Dict[int, float], Dict[str, Any], int, bool, float, int
    ]
) -> Dict[str, Any]:
    """
    ...
    y_pred_proba = model.predict_proba(X_test)[
        :, 1
    ]  # Probability of positive class (trade)
    ...
    if eval_economic:
        economic_metrics = calculate_economic_metrics(
            y_test, y_pred_proba, profit_threshold
        )
...
def run_walk_forward_validation(
    ...
    profit_threshold: Optional[float] = None,
) -> Tuple[List[Dict], List[float], List[int], List[int]]:
    """
    ...
    if early_stopping_rounds is None:
        early_stopping_rounds = TRAINER_CONFIG["model_training"][
            "early_stopping_rounds"
        ]
    if profit_threshold is None:
        profit_threshold = TRAINER_CONFIG["model_training"]["economic_profit_threshold"]
    ...
    logging.info(
        f"Starting sequential walk-forward validation with {n_splits} folds (changed from parallel to avoid hanging)"
    )
    ...
    for task in fold_tasks:
        try:
            ...
            logging.info(
                f"Fold {fold_idx} completed - AUC: {auc:.4f}, F1: {f1:.4f}, PnL: {pnl:.4f}"
            )
        except Exception as e:
    ...
    for result in fold_results:
        ...
        metrics_only = {
            ...
            if k not in ["probabilities", "predictions", "true_labels"]
        }
...
def optimize_threshold(
    ...
    try:
        ...
        logging.warning(
            f"⚠️ Probability calibration failed: {e}. Using original model."
        )
    ...
    for threshold in thresholds:
        ...
        logging.info(
            f"Threshold {threshold:.2f}: PnL={economic_metrics['pnl']:.4f}, "
    ...
    best_threshold_result = max(
        threshold_results, key=lambda x: (x["sharpe"], x["pnl"])
    )
    ...
    logging.info(
        f"   Expected Max Drawdown: {best_threshold_result['max_drawdown']:.4f}"
    )
...
def save_model_and_results(
    ...
    overall_auc: float,
    overall_f1: float,
    overall_economic: Dict[str, float],
    class_weights: Dict[int, float],
    horizon: int,
    profit_threshold: float,
    include_fees: bool,
    ...
    try:
        ...
        ax.set_title(
            "Binary Confusion Matrix - Trade/No-Trade Model", fontsize=14, pad=20
        )
    ...
    results["confusion_matrix"] = {
        ...
        "precision_trade": float(cm[1, 1] / (cm[1, 1] + cm[0, 1]))
        if (cm[1, 1] + cm[0, 1]) > 0
        else 0.0,
        "recall_trade": float(cm[1, 1] / (cm[1, 1] + cm[1, 0]))
        if (cm[1, 1] + cm[1, 0]) > 0
        else 0.0,
    }
    ...
    if eval_economic:
        logging.info(
            f"Overall PnL: {overall_economic['pnl']:.4f}, Sharpe: {overall_economic['sharpe']:.4f}"
        )
    ...
    try:
        model_card = {
            ...
            "selected_features": results.get("metadata", {}).get(
...
def train_model_binary(
    ...
    # Step 2: Prepare data
    X, y, sample_weights_all, n_splits = prepare_data(
        df, feature_columns, label_col, n_splits
    )
    ...
    model_params = perform_hyperparameter_tuning(
        X, y, sample_weights_all, tune, n_trials
    )
    ...
    overall_economic = {}
    if eval_economic:
        overall_economic = calculate_economic_metrics(
            fold_true_labels, fold_probabilities, profit_threshold
        )
...
def setup_logging(logfile: str | None = None, level: int = logging.INFO):
    ...
    if logfile:
        ...
        try:
            ...
            logger.addHandler(
                fh
            )  # Use logger.addHandler instead of logging.getLogger().addHandler
            ...
            try:
                ...
                logging.warning(
                    f"Could not open specified log file {logfile}: {e}. Using fallback: {fallback_logfile}"
                )
            except Exception as fallback_e:
                logging.warning(
                    f"Could not create fallback log file {fallback_logfile}: {fallback_e}. Logging to console only."
                )


def main():
    parser = argparse.ArgumentParser(
        description="Train a binary classification model for trading decisions using walk-forward validation."
    )
    parser.add_argument(
        "--data", required=True, help="Path to CSV file containing OHLCV data"
    )
    parser.add_argument(
        "--output", required=True, help="Path to output .pkl model file"
    )
    parser.add_argument(
        "--up_thresh",
        type=float,
        default=0.005,
        help="Profit threshold for trade signals (fraction). Default 0.005 (0.5%)",
    )
    parser.add_argument(
        "--horizon",
        type=int,
        default=5,
        help="Forward return horizon (periods ahead). Default 5",
    )
    parser.add_argument(
        "--results",
    ...
    parser.add_argument(
        "--logfile",
        default=None,
        help="Optional path to a logfile. If set, logs are written to this file as well as console.",
    )
    parser.add_argument(
        "--tune",
        action="store_true",
        help="If set, run Optuna hyperparameter tuning before training the final model",
    )
    parser.add_argument(
        "--n_trials",
        type=int,
        default=25,
        help="Number of Optuna trials when --tune is set. Default 25",
    )
    parser.add_argument(
        "--feature_selection",
        action="store_true",
        help="If set, perform feature selection based on gain importance and retrain final model",
    )
    parser.add_argument(
        "--early_stopping_rounds",
        type=int,
        default=50,
        help="Early stopping rounds to pass to LightGBM fit. Default 50",
    )
    parser.add_argument(
        "--eval_profit",
        action="store_true",
        help="If set, evaluate and log economic metrics (PnL, Sharpe ratio)",
    )
    ...
    if df.empty:
        logging.warning(
            "No data available for training after preprocessing. Creating minimal synthetic dataset for testing."
        )
...
class EnsembleModel:
    ...
    def __init__(
        self, models: Optional[Dict[str, Any]] = None, meta_model: Optional[Any] = None
    ):
        """
    ...
    def estimate_uncertainty(self, X: pd.DataFrame) -> Dict[str, np.ndarray]:
        ...
        if not self.is_trained:
            raise ValueError(
                "Ensemble model must be fitted before uncertainty estimation"
            )
...
def train_ensemble_model(
    df: pd.DataFrame,
    save_path: str = "models/ensemble_model.pkl",
    ...
    results = {
        ...
        "overall_metrics": {"auc": float(overall_auc), "f1": float(overall_f1)},
        "cv_scores": cv_scores,
    ...
    logger.info(
        f"Ensemble training completed. Overall AUC: {overall_auc:.4f}, F1: {overall_f1:.4f}"
    )
