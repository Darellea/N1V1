...
def create_sample_data():
    ...
    data_dict['BTC/USDT'] = pd.DataFrame({
        ...
        'high': 50000 + base_trend * 1000 + btc_noise + abs(np.random.randn(n_points)) * 500,
        'low': 50000 + base_trend * 1000 + btc_noise - abs(np.random.randn(n_points)) * 500,
    ...
    data_dict['ETH/USDT'] = pd.DataFrame({
        ...
        'high': 3000 + base_trend * 100 + eth_noise + abs(np.random.randn(n_points)) * 100,
        'low': 3000 + base_trend * 100 + eth_noise - abs(np.random.randn(n_points)) * 100,
    ...
    data_dict['SOL/USDT'] = pd.DataFrame({
        ...
        'high': 100 + sol_trend * 20 + sol_noise + abs(np.random.randn(n_points)) * 5,
        'low': 100 + sol_trend * 20 + sol_noise - abs(np.random.randn(n_points)) * 5,
...
def main():
    ...
    optimizer.optimize = Mock(return_value={
        'rsi_period': 14,
        'overbought': 70,
        'oversold': 30
    })
    ...
    validation_metrics = [results.get('results', {}).get(pair, {}) for pair in validation_pairs]
    ...
    print(f"  Train Pair ({train_pair}) Sharpe: {train_metrics.get('sharpe_ratio', 0):.3f}")
    print(f"  Avg Validation Sharpe: {avg_sharpe:.3f}")
    print(f"  Sharpe Ratio Difference: {avg_sharpe - train_metrics.get('sharpe_ratio', 0):.3f}")

    print(f"  Train Pair ({train_pair}) Return: {train_metrics.get('total_return', 0):.3f}")
    print(f"  Avg Validation Return: {avg_return:.3f}")
    print(f"  Return Difference: {avg_return - train_metrics.get('total_return', 0):.3f}")
